...
<script>
  ...
  import { getAccomodation } from '@/api/accomodation.api';
  ...
  export default defineComponent({
    ...
    computed: {
      ...
      serviceImageUrl(){
        ...
        return mediaPrefixer.getUrl(this.service.files[0], 'accommodation')
      },
    },
    methods: {
      toggleBookmark(){
        if(!this.service?._meta.bookmarked?._id){
          createBookmark({ entityId: this.service._id, entityType: "accommodation" })
            .then(res => {
    ...
    components: { AppLayout, AccomodationServiceView } })
</script>
