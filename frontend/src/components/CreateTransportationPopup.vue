<template>
	<div class="create-service-popup">
		<div class="bg-white create-service-popup-body">
			<div class="px-2">
				<font-awesome-icon class="cursor-pointer icon-sm" :icon="['fas', 'xmark']" @click="closeCreateTransportationPopup"/>
			</div>
			<div class="divider"></div>
			<div v-if="formStep === 1" class="px-2 pb-2 create-service-field-ctn">
				<label class="pt-2 pb-1">Transportation Type</label>
			...
			<div v-if="formStep === 2 && images.length" class="px-2 py-2 create-service-field-ctn">
				<div class="w-100 d-flex flex-column gap-2">
					<!-- <img v-for="url in images" :src="url" class="w-100"/> -->
			...
			<!-- <div v-if="formStep === 2" class="px-2 py-2 create-service-field-ctn">

				<div class="w-100 d-flex flex-column gap-2">
					<div class="w-100">
						<div class="w-100 position-relative">
							<div class="w-100 d-flex align-items-center justify-content-end py-2 px-2 create-service-top-bar">
								<font-awesome-icon class="icon-sm" :icon="['fas', 'chevron-up']"/>
								<font-awesome-icon class="icon-sm" :icon="['fas', 'chevron-down']"/>
								<font-awesome-icon class="icon-sm" :icon="['fas', 'trash-can']"/>
							</div>
						</div>
						<img src="/svgs/image_placeholder.svg" class="w-100 create-service-img-preview"/>
					</div>
				</div>
...
<script>
  ...
  export default defineComponent({
		...
		props: ["closeCreateTransportationPopup"],
		data(){
			return {
